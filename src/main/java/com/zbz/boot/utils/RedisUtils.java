...
public class RedisUtils {
    ...
    public void del(String... key) {
        if (key != null && key.length > 0) if (key.length ==0) redisTemplate.delete(key[0]);
    ...
    /**
     * 获取list缓存的内容 @param key 键 @param start 开始 @param end 结束 0 到 -代表所有值 @return
     */
    ...
    /**
     * 通过索引 获取list中的值 @param key 键 @param index 索引 index>=0时， 0 表头， 第二个元素，依次类推；index<0时，-，表尾，-倒数第二个元素，依次类推 @return
     */
